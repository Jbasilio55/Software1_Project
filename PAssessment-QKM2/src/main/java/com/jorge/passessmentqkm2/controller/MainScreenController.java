 ...
 *
 * FUTURE ENHANCEMENT: Improve the search function, so it is not case-sensitive.
 * FUTURE ENHANCEMENT: Add a database so the data can persist.
 *
...
public class MainScreenController implements Initializable {
    ...
    void onActionProductSearch(ActionEvent event){
        String text = productsSearchTxt.getText().trim();
        ObservableList<Part> allParts = Inventory.getAllParts();
        //make a new list that will hold all matches
        ObservableList<Product> searchedProducts = Inventory.lookupProduct(text);
        //set productsTableView to all matches
        ...
        try{
            ...
            if(searchedProducts.size() == 0){
                //change the text to a numeric form
                int searchedId = Integer.parseInt(text);
                //if any matches are found, add to searchedProducts
     ...
     * Populates the tables from static data created in MainApplication.
     *
